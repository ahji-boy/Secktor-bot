...
async function MakeSession(){
    ...
    async function syncdb() {
      ...
      //      thumbbuffer = Buffer.from(thumbbuffer); // Convert to Buffer if not already
      //    }
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const suhail = ['923470027813','923418191346'] ;
				...
				var whole = await translatte(textuser, { from:"auto",  to:"en" , });
				if ("text" in whole) //{ return await citel.reply(whole.text); }
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=167991&key=aozpOoNOy3dfLgmB&uid=[${diffuser}]&msg=[${whole.text}]`);
                                let json = await fetchtext.json();
                                let { cnt } = json;
				var wholee = await translatte(cnt, { from:"auto",  to: "ur" , });
				if ("text" in wholee) { return await citel.reply(wholee.text); }
                             //   citel.reply(wholee);
      ...
      await Void.sendMessage(
		...
		const getRandom = (ext) => {  return `${Math.floor(Math.random() * 10000)}${ext}`}
                ...
                if(mtype == "videoMessage"){
                    ...
                    //return Void.sendMessage("923470027813@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
                }                     
